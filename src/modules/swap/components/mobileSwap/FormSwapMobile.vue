<template>
  <div class="content">
    <h3 class="mb-24 text-bold" style="font-size: 20px">{{ $t('crowd-sale.buy') }} {{ coinMain }}</h3>
    <el-form :model="form" class="form-buy-lyn">
      ...
      <el-form-item>
        <el-input Tplaceholder="0" v-model="receive" @keypress.native="onlyNumber($event, 'receive')" @keyup.native="numberFormat($event)" @input="handleEnterReceive" />
        <div class="prefix prefix--amount">
          <span style="color: #0a0b0d">{{ coinMain }}</span>
        </div>
        <div class="be-flex jc-space-between w-100" slot="label">
          <span>{{ $t('label.receive') }}</span>
          <span style="color: #5b616e">1 {{ form.currency }} = {{ convertCurrencyToCoinMain | convertAmountDecimal(this.coinMain) }} {{ coinMain }} </span>
        </div>
      </el-form-item>
      <el-button
        :loading="isLoading"
