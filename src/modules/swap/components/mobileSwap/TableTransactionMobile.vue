  ...
  <div class="w-100 table-transaction">
    ...
    <div class="w" style="display: flex; flex-direction: column">
      <div class="container" v-for="data in data" :key="data.email">
        <div class="column-1">
          <p class="userEmail text-semibold">{{ data.transactionCode | formatTransactionCode(5, 5) }}</p>
          <p class="date">{{ data.transactionDate | formatMMDDYY }}</p>
        </div>
        <div class="column-2">
          <p class="amount-decrease paid">-{{ data.paidAmountDisplay | convertAmountDecimal(data.paidCurrency) }} {{ data.paidCurrency }}</p>
          <p class="amount-increase token">+{{ data.tokenAmountDisplay | convertAmountDecimal(data.tokenCurrency) }} {{ data.tokenCurrency }}</p>
        </div>
      </div>
      <el-pagination
        ...
        background
        class="pagination"
        popper-class="popper-paging-swap"
        :total="query.total"
        pager-count="3"
      >
      </el-pagination>
