<template>
  <div>
    <div v-if="isDesktop" class="be-flex align-center jc-space-between filter-collection">
        ...
        <el-form class="be-flex form-filter" @submit.prevent.native="handleSubmit">
          ...
          <el-form-item class="mr-16" v-if="tabActive === 'NFT'">
            <el-select
              ...
              :placeholder="$t('nft.place-holder.sale-status')"
              @change="handleChangeStatus"
              clearable
            >
          ...
          <el-form-item class="mr-16" v-if="tabActive === 'Collection'">
            <el-select
              @click.native="closePopper"
          ...
          <!-- <el-form-item v-if="isBuy && isBid && tabActive !== 'NFT'" class="mr-16">
            <el-select v-model="filter.saleType" @change="handleChangeStatus" :placeholder="$t('placeholder.sale-type')" clearable>
              <el-option v-for="item in getArrFilterSaleStatus" :key="item.id" :label="item.title" :value="item.value" />
            </el-select>
          </el-form-item> -->
            ...
            >
              <div class="wrap-inside">
                ...
                <div class="be-flex jc-flex-end footer">
                  <button class="btn-default mr-15 nft-header6 text-desc text-semibold w-auto" @click="handleReset">
                    <p>{{ $t('button.reset') }}</p>
                  </button>
          ...
          <el-form-item v-if="tabActive === 'NFT' && isBid && isBuy">
            <el-popover
              :value="isVisibleMoreFilter"
              placement="bottom-end"
              width="384"
              trigger="click"
              class="wrap-property"
              @show="handleShowMoreFilter"
              @hide="isVisibleMoreFilter = false"
              ref="morefilter"
            >
              <div class="wrap-inside">
                <div class="wrap-form">
                  <el-form>
                    <el-form-item v-if="isBuy && isBid && tabActive === 'NFT'">
                      <el-select v-model="filter.saleType" :placeholder="$t('placeholder.sale-type')" clearable class="in-popover">
                        <el-option v-for="item in getArrFilterSaleType" :key="item.id" :label="item.title" :value="item.value" />
                      </el-select>
                    </el-form-item>
                    <el-form-item v-if="tabActive === 'NFT'">
                      <el-select v-model="filter.network" placeholder="Blockchain" clearable class="in-popover">
                ...
                <div class="be-flex jc-flex-end footer">
                  <button class="btn-default mr-15 nft-header6 text-desc text-semibold w-auto" @click="handleCancel">
                    <p>{{ $t('button.cancel') }}</p>
                  </button>
                  <button class="btn-default-bg mr-10 nft-header6 text-semibold btn is-none-border w-auto" @click="handleApplyMoreFilter">
                    {{ $t('button.apply') }}
                  </button>
                </div>
              </div>
              <el-form-item class="more-filter" slot="reference">
                <!-- @click.native="closePopper" -->
                <el-select
                  v-popover:morefilter
                  popper-class="d-none"
                  :placeholder="$t('placeholder.more-filter')"
                  clearable
                  v-model="filter.saleType"
                  :data-alive="!!filter.saleType"
                  @clear="handleClearMoreFilter"
                >
      ...
      <div class="be-flex align-center filter-collection__right" v-if="tabActive !== 'User'">
...
<script lang="ts">
  ...
  export default class FilterExploreNft extends Mixins(FilterPriceNft) {
    ...
    saleStatus: Array<Record<string, any>> = [
      {
        id: 1,
        title: this.$i18n.t('sale-status.on-sale'),
        value: 'ON_SALE'
      },
      {
        id: 2,
        title: this.$i18n.t('sale-status.on-auction'),
        value: 'ON_AUCTION'
      },
      {
        id: 3,
        title: this.$i18n.t('sale-status.has-offer'),
        value: 'HAS_OFFER'
      },
      {
        id: 4,
        title: this.$i18n.t('sale-status.not-for-sale'),
        value: 'NOT_FOR_SALE'
      }
    ]
    ...
    get getArrFilterSaleStatus(): Array<Record<string, any>> {
      const arr: Array<Record<string, any>> = [
        {
          id: 1,
          title: this.$i18n.t('sale-status.for-sale'),
          value: 'ON_SALE'
        }
      ]
      ...
      arr.push({
        id: 4,
        title: this.$i18n.t('text-nft.not-for-sale'),
        value: 'NOT_FOR_SALE'
      })
      return arr
    }

    get getArrFilterSaleType(): Array<Record<string, any>> {
      if (this.isNft) {
        if (this.isBid) {
          return [
            {
    ...
    handleShowMoreFilter(): void {
      this.isVisible = false
      this.isVisibleMoreFilter = true
    }
    handleClearMoreFilter(): void {
      this.handleChangeStatus()
    }
    handleApplyMoreFilter(): void {
