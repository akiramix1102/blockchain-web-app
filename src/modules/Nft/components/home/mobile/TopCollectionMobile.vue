<template>
  <div class="top-collection">
    <div class="flex-column align-center jc-space-between mb-24 title">
      <div class="text-1xl text-semibold flex-column align-center">
        <span style="margin-right: 4px"> {{ $t('nft.home.top-collec') }}</span>
        <base-icon icon="icon-gold-medal" size="24" class="lh-default" />
      </div>
      <title-tab :arrTab="arrTab" @clickTab="handleChangeTab" class="title-tab-mobile" />
    </div>
    <div class="top-list">
      <collection-item-child v-if="arrItem.length" :arrItem="arrItem" @click="handleClickBtn" class="collection-item-mobile" :showBtn="false" />
