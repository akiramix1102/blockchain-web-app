<template>
  <div id="trending-collection">
    <div class="container-nft">
      <!-- <div class="mt-60 mb-40 nft-header3 text-semibold">
        <div class="title-icon">
          <p style="margin-right: 8px">Trending Collection</p>
          <img src="../../../assets/images/nft/fire.png" class="one-collection-fire" alt="" />
        </div>
        
      </div> -->
      <div class="be-flex align-center jc-space-between mb-40 title">
        <div class="nft-header3 text-semibold be-flex align-center">
          <span>{{ $t('nft.home.trending-collect') }}</span>
          <!-- <base-icon icon="trending-collection" size="48" class="lh-default" /> -->
          <img style="margin-left: 8px" src="../../../assets/images/nft/trending.png" class="one-collection-fire" alt="" />
        </div>
        <title-tab :arrTab="arrTab" :trending="checkTrending" @clickTab="handleClickTab" />
      </div>
      <swiper v-if="trendingCollectionList.length" class="wrap-top-collection" :options="swiperOption" ref="swiperRef">
        <!-- one collection -->
        <swiper-slide class="one-collection" v-for="item in trendingCollectionList" :key="item.id">
      ...
      <div v-if="trendingCollectionList.length > 4" class="btn-crousel cursor prev" @click="handleClickArrow('prev')">
        <base-icon icon="icon-prev" size="32" class="d-iflex" />
      </div>
      <div v-if="trendingCollectionList.length > 4" class="btn-crousel cursor next" @click="handleClickArrow('next')">
...
<script lang="ts">
  ...
  import TitleTab from '../components/base/TitleTab.vue'
  ...
  @Component({ components: { Swiper, SwiperSlide, TitleTab, EmptyBlock } })
  export default class TrendingCollection extends Vue {
    language = ''
    ...
    async getListTrending(): Promise<void> {
      const params = {
      ...
      try {
        this.trendingCollectionList = await apiNft.getListCollection(params)
        const language = window.localStorage.getItem('bc-lang')!
        for (let i = 0; i < this.trendingCollectionList.length; i++) {
          const objDesc = JSON.parse(this.trendingCollectionList[i].shortDescription)
          const textDescription = objDesc[language].replace(/\\n/g, '<br>')
          this.trendingCollectionList[i] = {
            ...this.trendingCollectionList[i],
    ...
    trendingCollectionList: Array<Record<string, any>> = []
    async created(): Promise<void> {
      this.getListTrending()
      const list = await apiNft.getListCategoryFilter()
      this.filterCategoryList = list.filter(item => item.levelDepth == '0')

      for (let i = 0; i < this.filterCategoryList.length; i++) {
        this.filterCategoryList[i] = {
          ...this.filterCategoryList[i],
          title: this.filterCategoryList[i].categoryName,
          type: this.filterCategoryList[i].id
        }
        this.arrTab.push(this.filterCategoryList[i])
      }

      // this.arrTab.concat(this.filterCategoryList)
      console.log('this.arrTab', this.arrTab)
      // for(let i=0; i<= filter.length;i++){
      //   for(let j=0;j<=this.arrTab.length;j++){
