  ...
  <div id="sack-detail-nft" class="sack-detail-nft">
    ...
    <div class="ml-auto detail-nft-right">
      <div class="be-scroll-custom wrap-fixed-top">
            ...
            <div class="be-flex wrap">
              ...
              <el-dropdown trigger="click" @command="handleOpen">
                ...
                > -->
                  ...
                  <el-dropdown-item v-if="showBtnUseNft && checkExistMoreButton" class="text-drop-nft" style="font-size: 16px" command="use-nft">{{
                    $t('text-nft.use-nft')
                  }}</el-dropdown-item>
        ...
        <div class="detail-nft-price">
          <span class="price text-base" v-if="isShowTextBuy"
            >{{ nftItem.numOfOwners === 1 ? $t('nft.price') : $t('nft.price-starting-at') }}
            <span class="price-value text-base text-bg">${{ (nftItem.minPrice * exchangeRate) | convertAmountDecimal('USD') }} </span></span
          >
          ...
          <div>
            <span v-if="nftItem.totalListing === null" class="quantity text-base">0/{{ nftItem.totalSupply | formatNumber }} {{ $t('for-sale') }}</span>
            <span v-else class="quantity text-base">{{ nftItem.totalListing | formatNumber }}/{{ nftItem.totalSupply | formatNumber }} {{ $t('for-sale') }}</span>
          </div>
        </div>
        <div class="ck-text-description detail-nft-des" v-if="textDescription">
          <div id="text-description" :class="showClass ? 'show-more-desc' : null" style="line-height: 24px" v-html="textDescription"></div>
          <div id="read-more" v-if="numOfLine > 2" style="margin-top: 5px" :class="!showClass ? 'text-rotate' : null" @click="showClass = !showClass">
            <div>
        ...
        <div class="isPc">
          <div class="sack-owner-create">
            <div v-if="nftItem.numOfOwners && nftItem.dispStatus !== 'USED'" class="sack-owner">
      ...
      <div class="wrap-fixed-bottom">
        ...
        <div v-if="isShowHighestBid || isShowHighestOffer" class="highest-bid">
          ...
          <div class="be-flex align-center jc-space-between highest-bid__content">
            <div class="left">
              ...
              <span class="nft-body-base text-hyperlink cursor text-overflow-1 name" @click="handleShowAccount(nftItem.highestBidId)"
                >{{ nftItem && nftItem.highestBidName }}
              </span>
        ...
        <div class="detail-button" :class="{ russian: selectLanguage() === 'ru' }">
          ...
          <div v-if="showBtnBuyNow && !showBuyNowOption" class="detail-button-one w-100">
            <!-- <el-popover ref="popper" placement="bottom" trigger="hover" popper-class="popper-list-token-accept">
              <div class="list-token-accept">
                ...
                >
                  ...
                  <div class="nft-body-base text-overflow-1 right">
                    <div class="d-iflex" v-if="(item.discountType === 'PERCENT' && item.discountPercent) || item.discountAmount">
                      <p class="d-ib text-xs text-error">
                        {{ getDiscount(item) }}
                      </p>
                      <p class="d-ib text-xs text-line-through ml-1">({{ getPriceOfToken(item) }} {{ item.currency }})</p>
                    </div>
                    <span class="d-ib">{{ getPriceOfTokenWithDiscount(item) }} {{ item.currency }}</span>
                  </div>
                </div>
              </div>
              <el-button
              ...
              >
                <div class="be-flex align-center">
                  ...
                  <div class="be-flex arrow" style="height: 24px; margin-bottom: -2px" v-if="coinMain == 'LYNK'">
                    <div class="isNormal">
                  ...
                  <div class="be-flex arrow" style="height: 24px; margin-bottom: -2px" v-if="coinMain == 'CLM'">
                    <div class="isNormal">
                      <base-icon :icon="classSecondaryBuyNow ? 'icon-arrow-down-black' : 'icon-arrow-down-white'" class="d-iflex" size="20" />
                    </div>
                    <div class="isLinear">
            ...
            >
              <div class="be-flex align-center">
                <p>{{ $t('text-nft.buy-now-detail', { amount: getPriceNft }) }}</p>
              </div>
            </el-button>
          </div>
          ...
          <div class="detail-button-one w-100" v-if="showBtnUseNft && !checkExistMoreButton">
            <el-button class="nft-header6 text-semibold text-center btn-default bid el-button-custom" @click="openPopupUsedNFT">
              <p>{{ $t('text-nft.use-nft') }}</p>
            </el-button>
          </div>
        </div>
    ...
    <popup-change-price :nftItem="nftItem" @reload="$emit('reload')" />
    <popup-burn />
    ...
    <PopupUsedNFT :nftItem="nftItem" :info="emailVerified" :remainQuantity="remainQuantity" @updateRemainQuantity="$emit('reload')" />
  </div>
</template>
...
<script lang="ts">
  ...
  import PopupUsedNFT from './popup/PopupUsedNFT.vue'
  ...
  export default class HeaderDetailNFT extends Mixins(PopupMixin, ActionNftMixin, ShowBtnMixin) {
    ...
    @bcNft.State('listTransactionNft') listTransactionNft!: Array<Record<string, any>>
    @bcNft.State('statusApproveItem') statusApproveItem!: string
    @bcNft.State('transactionCodeItem') transactionCodeItem!: string | any
    ...
    emailVerified: Record<string, any> = {}
    isActive = 'schedule'
    // totalListing = 0
    read = false
    typePopup = 'OWNER'
    ...
    get totalCirculating(): number {
      console.log('a', (this.nftItem.totalSupply / this.nftItem.circulatingSupply) * 100)
      if (this.nftItem.circulatingSupply == 0) {
        return 0
      } else {
        return (this.nftItem.totalSupply / this.nftItem.circulatingSupply) * 100
      }
    }
    ...
    get eBrochures(): Array<Record<string, any>> {
      if (this.metaData && this.metaData.length) {
    ...
    get exchangeRateLynk(): number {
      if (this.nftItem?.currency && this.listExchangeRate.length) {
        const coin = filter(this.listExchangeRate, elm => elm.currency === 'LYNK')[0]
        return coin.exchangeRate
      }
      return 0
    }
    ...
    get getPriceNft(): string {
      // let price = (this.nftItem.minListingPrice * this.exchangeRate) / this.exchangeRateOfInputPayment
      // if (this.tokenSelect && this.listExchangeRate.length) {
      //   const tokenAccept = filter(this.listTokenAccept, elm => elm.currency === this.tokenSelect)[0]
      //   if (tokenAccept.discountType === 'PERCENT') {
      //     price = (this.nftItem.minListingPrice * this.exchangeRate * (1 - tokenAccept.discountPercent / 100)) / this.exchangeRateOfInputPayment
      //   } else if (tokenAccept.discountType === 'AMOUNT') {
      //     price = ((this.nftItem.minListingPrice - this.getDiscountAmount(tokenAccept.discountAmount, this.nftItem.currency)) * this.exchangeRate) / this.exchangeRateOfInputPayment
      //   }
      // }

      if (this.listExchangeRate.length && this.listTokenAccept.length) {
        let price = 0
        const tokenAccept = filter(this.listTokenAccept, elm => elm.currency === 'LYNK')
        if (tokenAccept.length) {
          if (tokenAccept[0].discountType === 'PERCENT') {
            price = (this.nftItem.minListingPrice * this.exchangeRate * (1 - tokenAccept[0].discountPercent / 100)) / this.exchangeRateLynk
          } else if (tokenAccept[0].discountType === 'AMOUNT') {
            price = ((this.nftItem.minListingPrice - this.getDiscountAmount(tokenAccept[0].discountAmount, this.nftItem.currency)) * this.exchangeRate) / this.exchangeRateLynk
          }
          return this.$options.filters?.convertAmountDecimal(price, 'LYNK', false, true)
        } else {
          return this.usdPriceNft
          //   if (tokenAccept.discountType === 'PERCENT') {
          //     price = (this.nftItem.minListingPrice * this.exchangeRate * (1 - tokenAccept.discountPercent / 100)) / this.exchangeRateOfInputPayment
          //   } else if (tokenAccept.discountType === 'AMOUNT') {
          //     price = ((this.nftItem.minListingPrice - this.getDiscountAmount(tokenAccept.discountAmount, this.nftItem.currency)) * this.exchangeRate) / this.exchangeRateOfInputPayment
    ...
    get usdPriceNft(): string {
      let price = this.nftItem.minListingPrice * this.exchangeRate
      console.log('>>> / file: HeaderDetailNFT.vue / line 815 / price', price)
      return `$${this.$options.filters?.convertAmountDecimal(price, 'USD')}`
    }
    ...
    async openPopupUsedNFT(): Promise<void> {
      const detail = await apiNft.getDetailNftItem(this.nftItem.id)
      this.remainQuantity = detail.nftItem.myRemainQuantity

      const result = await apiAuth.getInfo()
      this.emailVerified = result
      if (result.emailVerified == 0) {
        let message: any = ''
        message = this.$t('nft.notify.errorUsed')
    ...
    handleShowAccount(id: number): void {
    ...
    async handleBuyNow(): Promise<void> {
      console.log('>>> / file: HeaderDetailNFT.vue / line 1156 / handleBuyNow')
      // try {
    ...
    async handleMakeOffer(): Promise<void> {
      try {
        ...
        if (this.nftItem.saleType == 'BUY' && this.nftItem.minListingId == null) {
          ...
          if (this.listAcceptCurrency.length) {
            await this.setTokenNeedGetBalance(this.listAcceptCurrency[0].currency)
            await this.makeOffer(this.nftItem, this.owners, this.contractAddress)
          } else {
            const message = this.$t('not_support_currency') as string
            this.$message.error({ message, duration: 5000 })
          }
        }
      } catch (error) {
    ...
    handleClickFavourite(): void {
      if (!this.isLogin && !this.walletAddress) {
    ...
    getPriceOfToken(item: Record<string, any>): string {
      let exchangeRateOfToken = 0
      if (this.listExchangeRate.length) {
        const coin = filter(this.listExchangeRate, elm => elm.currency === item.currency)[0]
        exchangeRateOfToken = coin.exchangeRate
      }
      const amount = (this.nftItem.minListingPrice * this.exchangeRate) / exchangeRateOfToken
      return this.$options.filters?.convertAmountDecimal(amount, item.currency)
    }
    getPriceOfTokenWithDiscount(item: Record<string, any>): string {
      let exchangeRateOfToken = 0
      if (this.listExchangeRate.length) {
        const coin = filter(this.listExchangeRate, elm => elm.currency === item.currency)[0]
        exchangeRateOfToken = coin.exchangeRate
      ...
      if (item.discountType === 'PERCENT') {
        priceWithDiscount = (this.nftItem.minListingPrice * this.exchangeRate * (1 - item.discountPercent / 100)) / exchangeRateOfToken
      } else if (item.discountType === 'AMOUNT') {
        let discountAmount = this.getDiscountAmount(item.discountAmount, this.nftItem.currency)
        priceWithDiscount = ((this.nftItem.minListingPrice - discountAmount) * this.exchangeRate) / exchangeRateOfToken
      }
      return this.$options.filters?.convertAmountDecimal(priceWithDiscount, item.currency)
    }
