...
const mutations: MutationTree<IAuth> = {
  ...
  SET_CONFIG_MODULE: (state, listModule) => {
    forEach(listModule, module => {
      ...
      } else if (module.displayStatus === 'ON' && module.status === 'BETA') {
  ...
  SET_CUSTOM_MODULE_CONFIG: (state, config) => {
    if (config.displayStatus === 'ON' && config.status === 'GOLIVE') {
      state.moduleConfig[config.moduleCode] = true
    } else if (config.displayStatus === 'ON' && config.status === 'BETA') {
      state.moduleConfig[config.moduleCode] = includes(config.listUserIds, state.user.userId)
    } else {
      state.moduleConfig[config.moduleCode] = false
    }
  },
