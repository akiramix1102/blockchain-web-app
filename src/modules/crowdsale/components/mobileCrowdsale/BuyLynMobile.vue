<template>
  <div class="content">
    <h3 class="mb-24 text-bold" style="font-size: 20px">{{ $t('crowd-sale.buy') }} {{ coinMain === 'LYNK' ? 'LYNK' : 'CLM' }}</h3>
    <el-form :model="buyLyn" class="form-buy-lyn">
      ...
      <el-form-item>
        ...
        />
        <div class="prefix prefix--amount">
          <span style="color: #0a0b0d">{{ coinMain === 'LYNK' ? 'LYNK' : 'CLM' }}</span>
        </div>
        <div class="be-flex jc-space-between w-100" slot="label">
          <span>{{ $t('label.receive') }}</span>
          <span style="color: #5b616e">1 {{ getUnit }} = {{ fillValue }} {{ coinMain === 'LYNK' ? 'LYNK' : 'CLM' }} </span>
        </div>
      </el-form-item>
      <el-button
        :loading="isLoading"
...
<script lang="ts">
  ...
  @Component
  export default class BuyLynMobile extends Vue {
