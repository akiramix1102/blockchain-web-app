...
export class SettingRepository extends BaseRepository {
  ...
  async updateProfileExternalWallet(data?: Record<string, any>): Promise<any> {
    try {
      const result = await request.put(`${this.prefix}/profile`, data)
      return Promise.resolve(result.data.data)
    } catch (error) {
      return Promise.reject(error)
    }
  ...
  async securityCode(data?: Record<string, any>): Promise<any> {
    try {
      const result = await request.post(`${this.prefix}/sendCode`, data)
      return Promise.resolve(result.data.data)
    } catch (error) {
      return Promise.reject(error)
    }
