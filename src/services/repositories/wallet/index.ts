...
export default class WalletRepository extends BaseRepository {
  ...
  async swapToken(body: Record<string, any>): Promise<any> {
    try {
      const result = await request.post(`${this.prefix}/swap`, body)
      return Promise.resolve(result)
    } catch (error) {
      return Promise.reject(error)
    }
  ...
  async getListTransactionStaking(params: Record<string, any>): Promise<any> {
    try {
      const rs = await request.get(`${this.prefix}/staking-transactions/wallet`, { params })
      return Promise.resolve(rs.data.data)
    } catch (error) {
      return Promise.reject(error)
    }
  ...
  async getInfoVesting(params?: Record<string, any>): Promise<Record<string, any>> {
    try {
      console.log(`${this.prefix}/vesting`)
