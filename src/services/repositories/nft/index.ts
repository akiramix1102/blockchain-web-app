...
export default class NftRepository extends BaseRepository {
  ...
  async getListBannerAdversiment(): Promise<Record<string, any>> {
    try {
      const result = await request.get(`${this.prefix}/home/banner-adversiment`)
  ...
  async useNFT(data: Record<string, any>): Promise<any> {
    try {
      const result = await request.post(`${this.prefix}/use-nft`, data)
  ...
  async getStakingChart(params: Record<string, any>): Promise<any> {
    try {
      const result = await request.get(`${this.prefix}/chart`, { params })
      return Promise.resolve(result.data.data)
    } catch (error) {
      return Promise.reject(error)
    }
