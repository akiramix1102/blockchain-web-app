  ...
  error => {
    ...
    if (data.statusCode === 400 || data.httpStatus == 400) {
      ...
      if (data.status === 'EMAIL_ALREADY_EXISTS') {
        message = i18n.tc('notify.email-already-exist')
      }
      ...
      if (data.status === 'INVALID_REF_USER_CODE') {
        message = i18n.tc('notify_invalid_ref_code')
      }
      if (data.status === 'NOT_ENOUGH_BALANCE') {
        message = i18n.tc('notify.balance-not-enough')
      }

      if (includes(config.url, 'staking-transactions')) {
        if (data.status === 'STAKE_GOT_MAX_AMOUNT') {
          message = 'Amount must be greater than maximum amount'
        } else if (data.status === 'STAKE_GOT_MIN_AMOUNT') {
          message = 'Amount must not be greater than minimum amount'
        } else if (data.status === 'STAKE_GOT_LIMIT_AMOUNT') {
          message = 'Total amount has reached the limit'
        } else {
