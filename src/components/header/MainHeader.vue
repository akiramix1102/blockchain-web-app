  ...
  <div id="main-header" class="header h-100">
    <div class="body h-100">
      <div class="content-left be-flex align-center">
        ...
        <!-- <div class="content-left__item text-base cursor menu content-none" @click="handleCommand('swap')">
          <span :class="$route.name === 'Swap' ? 'active' : null">Buy LYNK</span>
        </div> -->

        <router-link v-if="moduleConfig.Swap && isLogin" tag="div" :to="{ name: 'Swap' }" class="content-left__item text-base cursor menu content-none router-header">
          <div class="wrap" @click="handleRedirectHome('Swap')">
            <p class="text-base">Buy LYNK</p>
          </div>
        </router-link>
        <div
        ...
        <div
          class="content-left__item text-base cursor menu content-none"
          v-if="moduleConfig.Staking"
          @mouseleave="moduleEarnData.isHover = false"
          @mouseover="moduleEarnData.isHover = true"
        >
          <!-- <p class="menu-market" :class="isActiveMarketplace ? 'active-market' : null">Marketplace</p> -->
          <el-popover
            placement="bottom"
            width="180"
            popper-class="popper-detail-category popper-detail-category--header popper-detail-category--earn"
            trigger="hover"
            @hide="moduleEarnData.isVisiblePopper = false"
            @show="moduleEarnData.isVisiblePopper = true"
            @click="$router.push({ path: '/earn' })"
          >
            <div class="popper-category">
              <div
                class="cursor category-item"
                :class="item.id == $route.params.id && $route.name === 'DetailCategory' ? 'cate-active' : null"
                v-for="item in moduleEarnData.popperItems"
                :key="item.id"
                @click="$router.push({ path: '/earn' })"
              >
                <div class="flex wrap-option">
            ...
            <div class="be-flex align-center value-category" slot="reference">
              <p class="menu-market" :class="isActiveMenuStaking ? 'active-market' : null">Earn</p>
              <base-icon
                :icon="moduleEarnData.isHover ? (coinMain === 'LYNK' ? 'icon-arrow-bg' : 'icon-arrow-bg-clm') : 'icon-arrow-down-black'"
                size="16"
                class="d-iflex icon-caret"
                style="padding-left: 2px"
                :class="moduleEarnData.isVisiblePopper ? 'rotate' : null"
              />
            </div>
          </el-popover>
        </div>
      ...
      <div class="content-right align-center content-none">
        <div class="content">
          ...
          <div v-if="!isLogin && !walletAddress" class="be-flex align-center jc-space-between mr-24 login" @click="handleCommand('login')">
            <base-icon icon="icon-wallet" size="28" style="display: inline-flex" />
            <span class="text-lg">{{ $t('leftMenu.connect') }}</span>
          </div>
    ...
    <div @click="handleToggle" class="navbar-menu-icon cursor" :class="isToggle ? 'change' : ''">
    ...
    <nav class="nav__mobile" :class="isToggle ? 'chageActive' : ''">
      <div class="nav__mobile-close">
        <!-- <img src="../../assets/image/iconX.png" alt="" /> -->
      </div>
      <el-collapse class="menu__mobile-items" v-model="activeName" accordion @change="handleChangeMarket">
        <div class="h-100">
          <div class="line-menu">
            <div class="collapse-item-ido menu-link-change" v-if="moduleConfig.Swap && isLogin" @click="handleRedirectHome('Swap')">Buy LYNK</div>

            <el-collapse-item title="Metamart" class="menu-mobile-market" name="2">
            ...
            <el-collapse-item title="Earn" class="menu-mobile-market" name="3" v-if="moduleConfig.Staking">
              <!-- <ul class="collapse-menu"> -->
              <div class="popper-category style-content-maket">
                <div
                  class="cursor category-item menu-link-change"
          ...
          <div class="container-nft">
            <div class="sign-in-out-mobile container-nft">
              ...
              <el-button v-if="!isLogin && !walletAddress" @click="handleCommand('login')" class="btn is-none-border cursor btn-h40 btn-default-bg btn-theme style-btn-mobile">
                <div class="be-flex be-flex align-center jc-space-center">
                  <base-icon icon="icon-wallet-white" class="d-iflex" />
                  <p class="ml-8">{{ $t('title-log-in') }}</p>
                </div>
              </el-button>
...
<script lang="ts">
  ...
  export default class MainHeader extends Mixins(OnChainMixin) {
    ...
    @bcAuth.Mutation('SET_CUSTOM_MODULE_CONFIG') setCustomModuleConfig!: (config: Record<string, any>) => void
    ...
    moduleEarnData: Record<string, any> = {
      isHover: false,
    ...
    get isActiveMenuStaking(): boolean {
      return includes(this.$route.path, '/earn/staking')
    }
    get isActiveMarketplace(): boolean {
      return includes(this.$route.path, '/metamart') && !includes(this.$route.path, '/metamart/my-profile')
    }
    ...
    handleClickCategory(id: number): void {
      this.isToggle = false
      if (this.$route.name === 'DetailCategory' && this.$route.params.id === id + '') {
        window.location.reload()
      } else {
        this.$router.push({ name: 'DetailCategory', params: { id: id + '' } }).catch()
      }
    }
    ...
    handleClickLogOut(): void {
      const whiteList = ['MarketPlace', 'TopCollection', 'DetailCollection', 'DetailCategory', 'DetailNftItem', 'UserProfile']
      if (this.walletAddress) {
        ...
        Moralis.User.logOut().then(() => {
          ...
          if (!includes(whiteList, this.$route.name)) {
            this.$router.push({ name: 'MarketPlace' }).then(() => location.reload())
          } else {
            location.reload()
          }
        })
    ...
    async created(): Promise<void> {
      await this.getCustomModuleConfig()
    ...
    private async getCustomModuleConfig(): Promise<void> {
      const rs = await apiParams.getConfigModule()
      const configStaking = rs.find(el => el.moduleCode === 'Staking')
      if (configStaking) this.setCustomModuleConfig(configStaking)
    }
    // mounted(): void {
    // this.$nextTick(() => {
    ...
    //       navMobile.classList.remove('chageActive')
    //       visibleMobile.classList.remove('change')
    //       console.log('menu2')
