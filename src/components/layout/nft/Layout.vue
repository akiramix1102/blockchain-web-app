<template>
  <el-container class="main-layout" :class="isLoading ? 'be-flex-center' : null">
    ...
    <popup-banner-ads :objectBannerAds="objectBannerAds" />
  </el-container>
</template>
<script lang="ts">
  ...
  import PopupBannerAds from '@/modules/Nft/components/base/PopupBannerAds.vue'
  ...
  export default class LayoutMarketplace extends Mixins(PopupMixin) {
    // @bcNft.Action('setListBannerAds') setListBannerAds!: (data: Record<string, any>) => void
    ...
    async getBannerAds(): Promise<void> {
      const listBannerAdversiment = await apiNft.getListBannerAdversiment()
      this.objectBannerAds = listBannerAdversiment.content[0]
      const isOpenAds = localStorage.getItem('isOpenBannerAds')
      if (isOpenAds && isOpenAds === 'YES' && listBannerAdversiment.totalElements > 0) {
        this.setOpenPopup({
          popupName: 'popup-banner-ads',
          isOpen: true
        })
        window.localStorage.removeItem('isOpenBannerAds')
      }
    }
