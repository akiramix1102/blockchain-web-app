import { countBy, filter, reduce } from 'lodash'
...
export default class ShowBtnMixin extends Vue {
  ...
  get showBtnUseNft(): boolean {
    const isOwner = this.nftItem.isOwner
    const isNftVoucher = this.nftItem.isNftVoucher
    const myRemainQuantity = this.nftItem.myRemainQuantity
    return isOwner === 'YES' && isNftVoucher === 'YES' && myRemainQuantity
  }

  get checkExistMoreButton(): boolean {
    const arrStatusBtn = [this.showBtnPutOnSale, this.showBtnRemoveOnSale, this.showBtnAcceptOffer, this.showBtnBuyNow]
    const existEle = filter(arrStatusBtn, elm => elm)
    return existEle.length >= 2
  }
}
